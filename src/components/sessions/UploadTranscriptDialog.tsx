...
import { FileUp, Upload, X, Loader2, File } from "lucide-react";
...
export function UploadTranscriptDialog({
  ...
  const handleSave = async () => {
    ...
    if (!content.trim()) {
      toast({
        title: "Ошибка",
        description: "Введите текст транскрипта или загрузите файл",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: "Успешно",
        description: "Транскрипт загружен",
      });
      ...
      toast({
        title: "Ошибка",
        description: error instanceof Error ? error.message : "Не удалось загрузить транскрипт",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
