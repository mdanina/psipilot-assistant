...
interface SessionCaseSummaryBlockProps {
  sessionId: string;
}

/**
 * Компонент для отображения и генерации AI-сводки по сессии
 */
export function SessionCaseSummaryBlock({ sessionId }: SessionCaseSummaryBlockProps) {
  ...
  const loadCaseSummary = async () => {
    try {
      ...
      // Получаем данные сессии из базы
  ...
  const handleGenerate = async () => {
    try {
      ...
      const result = await generateCaseSummary(sessionId);
      ...
      toast({
        ...
        description: `Создана на основе ${result.based_on_notes_count} клинических заметок сессии`,
      });
    } catch (error) {
      ...
      if (errorMessage.includes('Нет завершённых клинических заметок')) {
        userMessage = 'Для генерации сводки необходимо наличие хотя бы одной завершённой клинической заметки в этой сессии.';
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            AI-анализ на основе клинических заметок этой сессии
          </CardDescription>
        </CardHeader>
        <CardContent>
        ...
        {caseSummary ? (
          ...
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p className="text-sm mb-4">
              ...
              AI-анализа на основе клинических заметок этой сессии.
            </p>
            <p className="text-xs text-muted-foreground">
              Для генерации необходимо наличие хотя бы одной завершённой клинической заметки в этой сессии.
            </p>
          </div>
        )}
      </CardContent>
