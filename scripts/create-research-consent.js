...
/**
 * Скрипт для создания согласия на исследования для пациента
 * 
 * Использование:
 *   node scripts/create-research-consent.js <patient-id> [options]
 * 
 * Опции:
 ...
 * 
 * Примеры:
 *   node scripts/create-research-consent.js "patient-uuid-here"
 *   node scripts/create-research-consent.js "patient-uuid-here" --expires "2025-12-31T23:59:59Z"
 *   node scripts/create-research-consent.js "patient-uuid-here" --method "written" --purpose "Исследование эффективности терапии"
 */
...
function parseArgs() {
  ...
  if (args.length < 1) {
    ...
    console.log('\nИспользование:');
    console.log('  node scripts/create-research-consent.js <patient-id> [options]');
    console.log('\nОпции:');
    ...
    console.log('\nПримеры:');
    console.log('  node scripts/create-research-consent.js "patient-uuid-here"');
    console.log('  node scripts/create-research-consent.js "patient-uuid-here" --expires "2025-12-31T23:59:59Z"');
    process.exit(1);
  }

  const patientId = args[0];
  const options = {
    purpose: 'Использование полностью деидентифицированных медицинских данных для научных исследований и разработки методов лечения. Данные будут использованы только в анонимном виде, без возможности идентификации пациента.',
    expires: null,
    method: 'electronic',
...
async function createResearchConsent() {
  try {
    const { patientId, purpose, expires, method, url, serviceKey } = parseArgs();

    console.log('🔬 Создание согласия на исследования...\n');
    ...
    // Проверяем, есть ли уже активное согласие на исследования
    ...
    if (activeConsents.length > 0) {
      console.log('⚠️  Предупреждение: У пациента уже есть активное согласие на исследования');
    ...
    const consentData = {
      ...
      legal_basis: 'consent', // Для исследований требуется явное согласие
      status: 'active',
      ...
      third_party_sharing: true, // Исследователи - третьи лица
      third_parties: ['Исследователи (анонимные данные)'],
      notes: 'Создано через скрипт create-research-consent.js. Данные будут полностью деидентифицированы перед предоставлением исследователям.',
    };
    ...
    console.log('\n✅ Готово! Теперь исследователи могут получить доступ к деидентифицированным данным этого пациента.');
